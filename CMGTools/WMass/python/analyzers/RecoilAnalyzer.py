    ...
    def computeUprojectionsWRTmuons(self, event):
        if self.cfg_ana.event_type == "W":
            ...
            if self.cfg_comp.isMC:
                ...
                event.u_par_MC, event.u_perp_MC = compute_u_projections(p4Ref=Mgen_p4, p42=event.h_p4_nt)
    ...
    def process(self, iEvent, event):
        ...
        for particle in event.pf_candidates:
            ...
            if abs(particle.pdgId()) == 13:
                if 0.9*event.muon_W.pt() < pt < 1.1*event.muon_W.pt():
                    continue

            # print dir(particle.sourceCandidatePtr(0))
            # aux_dz = particle.dz( )
            # if flaggona==0:
            #     print aux_dz, particle.fromPV()
            #     flaggona = 1
